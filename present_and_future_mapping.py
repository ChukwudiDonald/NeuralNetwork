...
# Example usage
df = pd.read_csv(file_paths[3], encoding='utf-16', delimiter=';')
N = 10000
tolerance = 0.001
x, y = create_input_output(torch.tensor(df["Open"].values), N)  # You already have this part for preparing data
...
optim = torch.optim.SGD(ann_model.parameters(),lr= 0.0001)
...
plt.plot(num_epoch,test_loss.detach(),"ro")
plt.xlabel("Epoch")
plt.ylabel("Loss")
...
# time_sequence = np.arange(len(y))  # Time sequence based on the length of y
# plt.plot(time_sequence,y,'b-', label="RealData")
# plt.plot(time_sequence,predictions.detach(),'r-',label='Predictions')
# plt.title(f"prediction-data r={np.corrcoef(y.T,predictions.detach().T)[0,1]: .2f}")
# plt.legend()
# plt.show()
